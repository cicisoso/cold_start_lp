...
\paragraph{Inductive cold-start evaluation.}
To test whether SSL benefits generalize to true cold-start settings, we hold out 20\% of nodes entirely from training and evaluate link prediction to/from these unseen nodes. As shown in Table~\ref{tab:inductive}, CL provides substantial inductive gains across all datasets, with the largest improvements on CiteSeer (+11.3\% AUC, +10.5\% AP) and CS (+6.5\% AUC, +6.5\% AP). This confirms that the SSL-induced representation invariances generalize beyond the transductive setting to genuinely unseen nodes.

\begin{table}[t]
\centering
\small
\caption{Inductive evaluation: 20\% of nodes held out entirely. CL consistently improves prediction of links to unseen nodes.}
\label{tab:inductive}
\begin{tabular}{l cc cc c}
\toprule
& \multicolumn{2}{c}{Vanilla} & \multicolumn{2}{c}{GlobalCL} & \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Dataset & AUC & AP & AUC & AP & $\Delta$AUC \\
\midrule
Cora & .696 & .704 & .766 & .772 & +7.0\% \\
CiteSeer & .721 & .721 & .834 & .826 & +11.3\% \\
PubMed & .853 & .837 & .891 & .877 & +3.8\% \\
CS & .845 & .840 & .910 & .905 & +6.5\% \\
\bottomrule
\end{tabular}
\end{table}
